export const formatRelativeTime = (fromDate: Date, toDate: string): string => {
...
export const formatSpecificRelativeDate = (now: Date, toDate: string): string => {
  const date = new Date(toDate);
  const tomorrow = new Date(now);
  tomorrow.setDate(tomorrow.getDate() + 1);
  const yesterday = new Date(now);
  yesterday.setDate(yesterday.getDate() - 1);
  const timeOptions: Intl.DateTimeFormatOptions = { hour: 'numeric', minute: '2-digit', hour12: true, timeZone: 'UTC' };

  if (
    date.toLocaleDateString(navigator.language, { timeZone: 'UTC' }) ===
