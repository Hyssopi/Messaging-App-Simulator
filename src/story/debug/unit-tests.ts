...
import { INSTANT, SLOWEST, SLOW, NORMAL, FAST } from '../../common/constants';
import { OCTOBER, NOVEMBER } from '../../date/constants';
import { state } from '../../state/state';
import { messageSpeedSetting } from '../../ui/ui';

export const unitTest = async (): Promise<void> => {
  // 1. Add Contact(s)
...
Should print an error in console...`,
    ...
    await mediaRight('src/story/debug/images/Cat.png', JaneDoe, undefined, SLOWEST);
    ...
    await mediaRight('src/story/debug/images/Cat.png', JaneDoe, undefined, SLOW);
    await textLeft(`TEST: Sent media with NORMAL delay...`, JaneDoe, INSTANT);
    await mediaRight('src/story/debug/images/Cat.png', JaneDoe, undefined, NORMAL);
    await textLeft(`TEST: Sent media with FAST delay...`, JaneDoe, INSTANT);
    await mediaRight('src/story/debug/images/Cat.png', JaneDoe, undefined, FAST);
    await textLeft(`TEST: Sent media with INSTANT delay...`, JaneDoe, INSTANT);
    await mediaRight('src/story/debug/images/Cat.png', JaneDoe, undefined, INSTANT);
...
With new line.`,
    ...
    notification('Notification Test 16', JaneDoe, undefined, new Date(state.gameState.date.getTime() - 1 * 1000));
    await textLeft(`TEST: Notification with time 30 seconds ago...`, JaneDoe, SLOWEST);
    notification('Notification Test 17', JaneDoe, undefined, new Date(state.gameState.date.getTime() - 30 * 1000));
    await textLeft(`TEST: Notification with time 30 minutes ago...`, JaneDoe, SLOWEST);
    notification('Notification Test 18', JaneDoe, undefined, new Date(state.gameState.date.getTime() - 30 * 60 * 1000));
    await textLeft(`TEST: Notification with time 1 hour ago...`, JaneDoe, SLOWEST);
    notification(
      'Notification Test 19',
      JaneDoe,
      undefined,
      new Date(state.gameState.date.getTime() - 1 * 60 * 60 * 1000),
    );
    ...
    notification(
      ...
      JaneDoe,
      undefined,
      new Date(state.gameState.date.getTime() - 1 * 24 * 60 * 60 * 1000),
    );
    ...
    notification(
      ...
      JaneDoe,
      undefined,
      new Date(state.gameState.date.getTime() - 1 * 7 * 24 * 60 * 60 * 1000),
    );
    ...
    notification(
      ...
      JaneDoe,
      undefined,
      new Date(state.gameState.date.getTime() - 1 * 31 * 24 * 60 * 60 * 1000),
    );
    await textLeft(`TEST: Notification with time 30 minutes from now...`, JaneDoe, SLOWEST);
    notification('Notification Test 23', JaneDoe, undefined, new Date(state.gameState.date.getTime() + 30 * 60 * 1000));
